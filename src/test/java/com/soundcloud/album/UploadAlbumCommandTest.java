...
public class UploadAlbumCommandTest {
    @Mock
    ...
    @Test
    public void uploadAlbumCommandExecuteIsRight() throws IOException, ServiceException, ServletException {
        user.setArtistId(1L);
        user.setId(1L);
        album.setId(1L);
        ...
        Mockito.doReturn(1L).when(albumService).uploadAlbum(Mockito.any(),Mockito.anyList(),Mockito.anyLong());
        Mockito.doNothing().when(request).setAttribute(ApplicationConstants.REQUEST_AUTHORIZED_ATTRIBUTE, false);

        Mockito.doReturn(requestDispatcher).when(request).getRequestDispatcher("?command="+CommandType.GET_ALL_USER_ARTIST_ALBUMS_COMMAND);
        Mockito.doNothing().when(requestDispatcher).forward(request, response);
        Command command = new UploadAlbumCommand(albumService);
        command.execute(request, response);
        Mockito.verify(albumService).uploadAlbum(Mockito.any(),Mockito.anyList(),Mockito.anyLong());
