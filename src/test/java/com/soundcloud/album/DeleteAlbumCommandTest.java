...
public class DeleteAlbumCommandTest {
    @Mock
    ...
    @Test
    public void deleteAlbumCommandExecuteIsRight() throws IOException, ServiceException {
        ...
        Mockito.doReturn(true).when(albumService).deleteAlbum(Mockito.any());

        Mockito.doReturn("").when(request).getContextPath();
        Mockito.doNothing().when(response).sendRedirect(
                "" + "?" + ApplicationConstants.COMMAND_NAME_PARAM + "=" + CommandType.GET_USER_LIBRARY_COMMAND);

        Command command = new DeleteAlbumCommand(albumService);
        command.execute(request, response);
        Mockito.verify(albumService).deleteAlbum(Mockito.anyLong());
