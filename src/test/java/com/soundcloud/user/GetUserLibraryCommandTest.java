...
public class GetUserLibraryCommandTest {
    @Mock
    ...
    public GetUserLibraryCommandTest() throws IOException {
    ...
    @Test
    public void getUserLibraryCommandExecuteIsRight() throws ServiceException, IOException, ServletException {

        SecurityContext.getInstance().authorize(dto, "");
        Mockito.doReturn(dto).when(userService).getUserLibrary(Mockito.any());
        ...
        Mockito.doNothing().when(request).setAttribute(ApplicationConstants.REQUEST_PARAMETER_USER, dto);
        Mockito.doReturn(requestDispatcher).when(request).getRequestDispatcher(ApplicationPage.LIBRARY.getPagePath());
        Mockito.doNothing().when(requestDispatcher).forward(request, response);
        Command command = new GetUserLibraryCommand(userService);
        command.execute(request, response);
        Mockito.verify(request).setAttribute(ApplicationConstants.REQUEST_PARAMETER_USER, dto);
