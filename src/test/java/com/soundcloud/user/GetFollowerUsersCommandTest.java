...
public class GetFollowerUsersCommandTest {
    @Mock
    ...
    public GetFollowerUsersCommandTest() throws IOException {
    ...
    @Test
    public void getFollowerUsersCommandExecuteIsRight() throws ServiceException, IOException {
        SecurityContext.getInstance().authorize(dto, "");
        List<UserDto> userDtos = new LinkedList<>();
        Mockito.doReturn(userDtos).when(userService).getFollowerUsers(Mockito.any());
        Mockito.doReturn(session).when(request).getSession();
        Mockito.doReturn("").when(session).getId();

        Mockito.doNothing().when(request).setAttribute("followerUsers", userDtos);
        ...
        Command command = new GetFollowerUsersCommand(userService);
        command.execute(request, response);
        Mockito.verify(request).setAttribute("followerUsers", userDtos);
