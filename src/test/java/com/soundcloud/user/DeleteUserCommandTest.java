...
public class DeleteUserCommandTest {
    ...
    public DeleteUserCommandTest() throws IOException {
    ...
    @Test
    public void deleteUserCommandExecuteIsRight() throws ServiceException, IOException {
        user.setArtistId(1L);
        ...
        Mockito.doReturn(true).when(userService).deleteUser(Mockito.anyLong());

        Mockito.doReturn("").when(request).getContextPath();
        Mockito.doNothing().when(response).sendRedirect(
                ""
                        + "?" + ApplicationConstants.COMMAND_NAME_PARAM + "=" + CommandType.GET_ALL_USER_ARTIST_ALBUMS_COMMAND);
