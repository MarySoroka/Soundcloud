...
public class SubscribeUserCommandTest {
    @Mock
    ...
    public SubscribeUserCommandTest() throws IOException {
    ...
    @Test
    public void subscribeUserCommandExecuteIsRight() throws IOException, PasswordEncodingException, ValidatorException, ServletException, ServiceException {
        ...
        Mockito.doReturn(true).when(userService).subscribeUser(Mockito.anyLong(), Mockito.any(BigDecimal.class));

        Mockito.doNothing().when(userService).addUserWallet(Mockito.anyLong(), Mockito.any());

        Mockito.doReturn(new BigDecimal(10)).when(userService).getUserWalletAmount(Mockito.anyLong());

        Mockito.doReturn(session).when(request).getSession();
        Mockito.doReturn("").when(session).getId();
        Mockito.doNothing().when(request).setAttribute(Mockito.anyString(), Mockito.any());
        Mockito.doReturn(requestDispatcher).when(request).getRequestDispatcher("?" + ApplicationConstants.COMMAND_NAME_PARAM +
        ...
        Command command = new SubscribeUserCommand(userService);
