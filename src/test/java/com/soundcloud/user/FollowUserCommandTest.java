...
public class FollowUserCommandTest {
    ...
    private final User user = new User(1L,
            ...
            12,
            15, 1L);
    public FollowUserCommandTest() throws IOException {
    ...
    @Test
    public void followCommandExecuteIsRight() throws ServiceException, IOException {
        ...
        Mockito.doReturn(true).when(userService).followUser(1L,12L,2L,3L);
        ...
        Command command = new FollowUserCommand(userService);
        command.execute(request,response);
        Integer userFollows = SecurityContext.getInstance().getCurrentUser().getUserFollows();
        Assert.assertEquals(new Integer(13), userFollows);
    }
}
