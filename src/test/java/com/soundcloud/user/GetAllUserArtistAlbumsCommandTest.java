...
public class GetAllUserArtistAlbumsCommandTest {
    @Mock
    ...
    public GetAllUserArtistAlbumsCommandTest() throws IOException {
    ...
    @Test
    public void getAllUserArtistAlbumsCommandExecuteIsRight_whenUserAuthorized() throws ServiceException, IOException, ServletException {
        SecurityContext.getInstance().authorize(dto,"");
        List<UserDto> users = new LinkedList<>();
        Mockito.doReturn(users).when(userService).getAllArtistAlbums(Mockito.anyLong());
        Mockito.doReturn(users).when(userService).getNotFollowUsers(Mockito.anyLong());
        Mockito.doReturn(dto).when(userService).getUserLikedAlbums(Mockito.anyLong());
        Mockito.doReturn(session).when(request).getSession();
        Mockito.doReturn("").when(session).getId();

        Mockito.doNothing().when(request).setAttribute("user", dto);
        Mockito.doNothing().when(request).setAttribute("notFollowUsers", users);
        Mockito.doNothing().when(request).setAttribute("users", users);
        ...
        Command command = new GetAllUserArtistAlbumsCommand(userService);
        command.execute(request, response);
        Mockito.verify(request).setAttribute("user", dto);
        Mockito.verify(request).setAttribute("users", users);
        Mockito.verify(request).setAttribute("notFollowUsers", users);

    }
    @Test
    public void getAllUserArtistAlbumsCommandExecuteIsRight_whenUserNotAuthorized() throws ServiceException, IOException, ServletException {
        List<UserDto> users = new LinkedList<>();
        Mockito.doReturn(users).when(userService).getAllArtistAlbums();
        Mockito.doReturn(session).when(request).getSession();
        Mockito.doReturn("").when(session).getId();
        Mockito.doNothing().when(request).setAttribute("users", users);
        request.setAttribute(ApplicationConstants.REQUEST_AUTHORIZED_ATTRIBUTE, false);
