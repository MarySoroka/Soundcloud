...
public class RegisterUserCommandTest {
    ...
    public RegisterUserCommandTest() throws IOException {
    ...
    @Test
    public void registerCommandExecuteIsRight() throws IOException, PasswordEncodingException, ValidatorException, ServletException, ServiceException {
        ...
        Mockito.doReturn(user.getEmail()).when(request).getParameter(ApplicationConstants.REQUEST_PARAMETER_EMAIL);

        user.setPassword(SecurityUtil.encode(user.getPassword()));
        Mockito.doReturn(true).when(userService).registerUser(Mockito.any());
        ...
        Command command = new RegisterUserCommand(beanValidator, userService);
