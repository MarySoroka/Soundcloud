...
public class GetFollowUsersCommandTest {
    @Mock
    ...
    public GetFollowUsersCommandTest() throws IOException {
    ...
    @Test
    public void getFollowUsersCommandExecuteIsRight() throws ServiceException, IOException {
        SecurityContext.getInstance().authorize(dto, "");
        List<UserDto> userDtos = new LinkedList<>();
        Mockito.doReturn(userDtos).when(userService).getFollowUsers(Mockito.any());
        Mockito.doReturn(session).when(request).getSession();
        Mockito.doReturn("").when(session).getId();

        Mockito.doNothing().when(request).setAttribute("followUsers", userDtos);
        ...
        Command command = new GetFollowUsersCommand(userService);
        command.execute(request, response);
        Mockito.verify(request).setAttribute("followUsers", userDtos);
