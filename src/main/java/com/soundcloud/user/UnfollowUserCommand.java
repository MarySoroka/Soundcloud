...
@Bean(beanName = "UNFOLLOW_USER_COMMAND")
public class UnfollowUserCommand extends AbstractCommand {
    private final UserService userService;
    private static final Logger LOGGER = LogManager.getLogger(UnfollowUserCommand.class);

    public UnfollowUserCommand(UserService userService) {
    ...
    public void executeCommand(HttpServletRequest request, HttpServletResponse response) throws CommandExecuteException {
        try {
            ...
            if (user != null) {
                ...
                Integer followersUserAmount = Integer.valueOf(request.getParameter(ApplicationConstants.REQUEST_FOLLOWER_USER_AMOUNT));
                Integer userFollows = user.getUserFollows();
                boolean follow = userService.unfollowUser(userId, userFollows, followsUserId, followersUserAmount);
                if (follow) {
                    --userFollows;
