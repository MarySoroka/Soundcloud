...
@Bean(beanName = "DELETE_LIKE_ALBUM_COMMAND")
public class DeleteLikeAlbumCommand extends AbstractCommand {
    private final AlbumService albumService;
    private static final Logger LOGGER = LogManager.getLogger(DeleteLikeAlbumCommand.class);

    public DeleteLikeAlbumCommand(AlbumService albumService) {
    ...
    public void executeCommand(HttpServletRequest request, HttpServletResponse response) throws CommandExecuteException {
        try {
            ...
            if (currentUser != null) {
                boolean isDelete = albumService.deleteLikedAlbum(albumId, currentUser.getId(), likesAmount);
                if (isDelete) {
                    redirect(response,
